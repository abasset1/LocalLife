...
Traiter LL-3012 — Backend : Intégration du géocodage.

Objectifs :

Intégrer une API de géocodage (ex : Nominatim) pour convertir une adresse envoyée par le frontend en `latitude`/`longitude` côté backend, avant sauvegarde en base (on ne stocke pas l'adresse). Gérer les erreurs (ex : adresse non trouvée).
